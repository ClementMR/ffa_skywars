...
function ffa_boss.join_player(player)
    ...
    if is_in_combat(name) then
        core.chat_send_player(name, ffa_boss.S("You cannot join the boss fight while in combat."))
        return false
    end

    if ffa_boss.state.loot_phase or not ffa_boss.get_boss() then
        core.chat_send_player(name, ffa_boss.S("The Forgotten is not fighting right now."))
        return false
    end

    if ffa_boss.is_member(name) then
        core.chat_send_player(name, ffa_boss.S("You are already in the boss fight."))
        return false
    end

    local max = ffa_boss.settings.max_players
    if ffa_boss.member_count() >= max then
        core.chat_send_player(name, ffa_boss.S("The boss fight already has @1 players.", max))
        return false
    end

    ffa_boss.state.members[name] = true
    player:set_pos(ffa_boss.get_position("player_spawn"))
    core.chat_send_player(name, ffa_boss.S("You joined the fight against The Forgotten."))
    return true
end
...
function ffa_boss.leave_player(player)
    ...
    core.chat_send_player(name, ffa_boss.S("You left the boss fight."))
    return true
end
...
core.register_on_joinplayer(function(player)
    ...
    then
        core.chat_send_player(player:get_player_name(), core.colorize("#d66823", "[Boss] ") .. 
            ffa_boss.S(
                "The Forgotten has appeared. Use @1 to fight it. Place left : @2", 
                core.colorize("cyan", "/boss join"), 
                (max_players - members_count)
            ))
    end
end)
