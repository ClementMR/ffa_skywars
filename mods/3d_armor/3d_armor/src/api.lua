...
armor.punch = function(self, player, hitter, time_from_last_punch, tool_capabilities)
	...
	local wear_multiplier = 1
	if type(tool_capabilities) == "table" then
		wear_multiplier = tonumber(tool_capabilities.armor_wear_multiplier) or 1
		wear_multiplier = math.max(0.1, math.min(3, wear_multiplier))
	end
	...
	for i, stack in pairs(list) do
		if stack:get_count() == 1 then
			...
			if damage == true then
				local scaled_use = math.max(1, math.floor(use * wear_multiplier + 0.5))
				self:damage(player, i, stack, scaled_use)
...
armor.get_armor_formspec = function(self, name)
    ...
    for a = 1, #armor.elements-1 do
        if player:get_inventory():get_stack("armor", a):is_empty() then
            armor_slot_imgs = armor_slot_imgs ..
                "image[0," .. (a - 1) .. ";1,1;3d_armor_stand_" ..
                armor.elements[a] .. ".png]"
        end
    end

    local form =
        "list[detached:" .. name .. "_armor;armor;0,0;1,5;]" ..
